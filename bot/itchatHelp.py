...
def init_wechat_config():
    ...
    my_set['wechat_nick_name'] = wechat_nick_name
    my_set['wechat_uuid'] = wechat_uuid

    # start---------------------------处理自动回复好友---------------------------start
    reply = my_set.get('auto_reply_info')
    ...
    # start ----------------------------------- 群功能初始化 ----------------------------------- start
    helper = my_set.get('group_helper_conf')
...
def log_all_config():
    ...
    """
    print('=' * 80)
    channel = config.get_config('auto_reply_info').get('bot_channel', 7)
    ...
    reply = config.get_config('auto_reply_info', None)
    ...
    # start ----------------------------------- 群功能日志说明 ----------------------------------- start
    helper = config.get_config('group_helper_conf')
    if not helper or not helper.get('is_open'):
        ...
        if helper.get('is_all'):
            ...
            nicknames = []
            for auid in auto_uuids:
                chat_rooms = itchat.search_chatrooms(userName=auid)
                nickname = chat_rooms['NickName']  # 群聊名称
                nicknames.append(nickname)
            nns = '，'.join(nicknames)
    ...
    alarm = config.get_config('alarm_info')
...
def is_white_group(uuid) -> bool:
    ...
    for au_id in auto_uuids:
        chat_rooms = itchat.search_chatrooms(name=au_id)
        print(f'''room: {chat_rooms}''')
